...
function export_plate_response_video( ...
    ...
    response_ylim_pre = compute_time_response_ylim(w_pre_response);
    response_ylim_post = compute_time_response_ylim(w_post_response);
    ...
    [h_pre_response, h_pre_marker] = plot_reference_response( ...
        ax_pre, t_eval, lambda(idx_pre_flutter), ...
        'before', style, response_ylim_pre, [0.30, 0.45, 0.80]);

    ax_post = nexttile(tl, 5);
    [h_post_response, h_post_marker] = plot_reference_response( ...
        ax_post, t_eval, lambda(idx_post_flutter), ...
    ...
    for frame_idx = 1:n_frames
        ...
        response_time_idx = animation_progress_time_index(frame_idx, n_frames, numel(t_eval));
        update_reference_response( ...
            h_pre_response, h_pre_marker, t_eval, w_pre_response, response_time_idx);
        update_reference_response( ...
            h_post_response, h_post_marker, t_eval, w_post_response, response_time_idx);
...
function export_cached_summary_video(params, plot_data, video_options)
    ...
    ax_pre = nexttile(tl, 4);
    [h_pre_response, h_pre_marker] = plot_reference_response( ...
        ax_pre, params.t_eval, lambda(idx_pre_flutter), ...
        'before', style, response_ylim_pre, [0.30, 0.45, 0.80]);

    ax_post = nexttile(tl, 5);
    [h_post_response, h_post_marker] = plot_reference_response( ...
        ax_post, params.t_eval, lambda(idx_post_flutter), ...
        'after', style, response_ylim_post, [0.80, 0.35, 0.10]);
    ...
    for lambda_idx = 1:stop_lambda_idx
        ...
        response_time_idx = animation_progress_time_index( ...
            lambda_idx, stop_lambda_idx, numel(params.t_eval));
        update_reference_response( ...
            h_pre_response, h_pre_marker, params.t_eval, w_pre_response, response_time_idx);
        update_reference_response( ...
            h_post_response, h_post_marker, params.t_eval, w_post_response, response_time_idx);
...
function [h_response, h_marker] = plot_reference_response( ...
    ax, t_eval, lambda_value, relation_label, style, response_ylim, line_color)
...
function update_reference_response(h_response, h_marker, t_eval, w_response, time_idx)
    time_idx = min(max(1, time_idx), numel(t_eval));
    idx_response = 1:time_idx;
    set(h_response, ...
        'XData', t_eval(idx_response), ...
        'YData', w_response(idx_response));
    set(h_marker, ...
        'XData', t_eval(time_idx), ...
        'YData', w_response(time_idx));
end


function time_idx = animation_progress_time_index(frame_idx, n_frames, Nt)
    if n_frames <= 1
        time_idx = Nt;
        return;
    end
    time_idx = 1 + round((frame_idx - 1) * (Nt - 1) / (n_frames - 1));
end
